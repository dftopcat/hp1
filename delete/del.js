;(function () {
    ...
    setTimeout(function () {
      ...
      console.log("%c     made by the @dftopcat     ", cstyle);
      console.log("https://twitter.com/dftopcat  https://youtube.com/dftopcat");
    }, 100);
  
    var el = document.querySelector('[data-dftopcat-credit]');
    ...
    if (el) {
      ...
      // if ( !hasText ) { 
      //   el.innerHTML = '<span>Watch the @dftopcat code this pen live!</span>';
      // }
  
      el.insertAdjacentHTML('afterbegin', `
  <div class="kf_credit_faces">
  <svg viewBox="0 0 122 68" xmlns="http://www.w3.org/2000/svg" fill-rule="evenodd" clip-rule="evenodd" stroke-linecap="round" stroke-miterlimit="1.5">
    ...
    <path d="M60.96 0L27.019 33.941 60.96 67.882l33.941-33.941L60.96 0z" fill="red"/>
    <path d="M70.56 43.541l-9.6-9.6 9.6-9.6M55.68 21.941V46.11" fill="none" stroke="#333" stroke-width="7.2" stroke-linecap="butt"/>
  </svg>
  </div>`);
  
      if (!hasText) {
        el.insertAdjacentHTML('afterbegin', '<span>Find out more about dftopcat</span> ');
        el.insertAdjacentHTML('beforeend', ' <span>software services consultancy</span>');
      }
  ...
  @dftopcat kf_credit_slide-in { from { transform: translateY(100%); opacity: 0; } }
  ...
  .kf_credit_face.\-shaw { right: auto; }
  .kf_credit_face.\-david { left: auto; transition-delay: 0.1s; z-index: 4; transform: translateZ(10px) rotateY(0deg) ; }
  
  .kf_credit:hover .kf_credit_face {
  transform: rotateY(360deg);
  }
  .kf_credit:hover .kf_credit_face.\-david {
  transform: translateZ(10px) rotateY(360deg);
      ...
      el.onclick = function () {
        if (ga) {
          ...
          ga('send', 'event', 'dftopcat', 'click', url, {
            'transport': 'beacon',
